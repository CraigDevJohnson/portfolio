...
- `projects-destinations` exposes only the destination links supplied by each project's data; the page footer has separate GitHub Profile and Experience actions.
...
- **Destinations.** Require the exact dossier action map: Personal Portfolio Website has external Live Demo and GitHub links; New User Account Provisioning has none; Soccer Schedule Scraper has an internal `/soccer` Live Demo and an external GitHub link. External links require `target="_blank"` plus `rel` containing `noopener` and `noreferrer`; `/soccer` must omit those attributes. Separately assert the footer GitHub Profile and Experience actions.
...
- A failed project image must reveal the dossier's local fallback without hiding its text.
