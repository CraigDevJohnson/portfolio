...
- **Interruption state.** Open `/__preview/portal/error`; expect HTTP `503` and the full operator interruption page. Capture this with an HTTP response body/status alongside browser evidence because a successful-looking screenshot cannot prove the status code.
