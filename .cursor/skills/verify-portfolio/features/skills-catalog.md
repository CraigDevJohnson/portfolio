...
- `skills-retry` keeps the current catalog visible after a filter request fails and retries the failed URL.
...
- **Failure and retry.** Intercept one `/skills/filtered` request with HTTP 503, trigger a filter, and require the existing results to remain visible with `The toolkit could not be updated` and `#skills-filter-retry`. Remove the interception, choose Retry, and require the intended filter and URL to succeed.
