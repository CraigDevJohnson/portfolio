...
mkdir -p "$fake_bin"
for command_name in gh aws curl docker sleep; do
...
promotion_deployment_json() {
  jq -nc --arg source_sha "$1" --arg digest "$2" '{
    id: 42,
    ref: $source_sha,
    sha: $source_sha,
    task: "portfolio-lambda-development",
    environment: "development",
    description: ("Lambda " + $digest),
    creator: {login: "github-actions[bot]", type: "Bot"}
  }'
}

promotion_status_json() {
  jq -nc --arg state "$1" --arg source_sha "$2" --arg digest "$3" '[{
    state: $state,
    environment: "development",
    environment_url: "https://dev.craigdevjohnson.com",
    description: ("Verified " + $source_sha + " at " + $digest),
    creator: {login: "github-actions[bot]", type: "Bot"}
  }]'
}
...
record_development_script=$(cat "$root_dir/scripts/record-ci-lambda-development.sh")
grep -Fq -- '-f task=portfolio-lambda-development' \
  "$root_dir/scripts/record-ci-lambda-development.sh"
assert_before \
  "$record_development_script" \
  "' > \"${literal_dollar}EVIDENCE_DIR/github-deployment.json\"" \
  "deployments/${literal_dollar}deployment_id/statuses"
grep -Fq 'if-no-files-found: error' << EOF
$development_job
EOF
