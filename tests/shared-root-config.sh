#!/bin/sh
set -eu

repo_root=$(CDPATH='' cd -- "$(dirname "$0")/.." && pwd)

fail() {
	printf 'FAIL: %s\n' "$1" >&2
	exit 1
}

test ! -e "$repo_root/infra/lambda.tf" || fail "legacy root still declares absent Lambda/API resources"

actual_resources=$(
	grep -hEo '^resource "[^"]+" "[^"]+"' "$repo_root"/infra/*.tf |
		sed 's/^resource "//; s/" "/./; s/"$//' |
		sort
)
expected_resources=$(printf '%s\n' \
	'aws_dynamodb_table.google_connections' \
	'aws_dynamodb_table.soccer_sessions' \
	'aws_ecr_lifecycle_policy.app' \
	'aws_ecr_repository.app' \
	'aws_iam_policy.google_connections_dynamodb' \
	'aws_iam_policy.soccer_sessions_dynamodb')
test "$actual_resources" = "$expected_resources" || {
	printf 'Unexpected shared-root managed resources:\n%s\n' "$actual_resources" >&2
	exit 1
}

actual_outputs=$(
	grep -hEo '^output "[^"]+"' "$repo_root"/infra/*.tf |
		sed 's/^output "//; s/"$//' |
		sort
)
expected_outputs=$(printf '%s\n' \
	'ecr_repository_url' \
	'google_connection_table_arn' \
	'google_connection_table_name')
test "$actual_outputs" = "$expected_outputs" || {
	printf 'Unexpected shared-root outputs:\n%s\n' "$actual_outputs" >&2
	exit 1
}

if grep -Eq '^  (deploy|redeploy|logs|deploy-lambda|redeploy-lambda|legacy-apprunner-retirement-(init|preflight|plan|apply)):' \
	"$repo_root/Taskfile.yaml"; then
	fail "Taskfile exposes a retired shared-stack deployment or App Runner interface"
fi

# The backticks are Markdown delimiters, not shell syntax.
# shellcheck disable=SC2016
advertised_tasks=$(
	grep -Eo '`task [a-z0-9-]+`' "$repo_root/AGENTS.md" |
		sed 's/^`task //; s/`$//' |
		sort -u
)
for task_name in $advertised_tasks; do
	task --dir "$repo_root" --summary "$task_name" >/dev/null 2>&1 ||
		fail "AGENTS.md advertises nonexistent task $task_name"
done

if grep -Eq '^variable "lambda_' "$repo_root/infra/variables.tf"; then
	fail "shared root still exposes variables for absent Lambda resources"
fi

grep -F 'version = "= 5.100.0"' "$repo_root/infra/versions.tf" >/dev/null ||
	fail "shared-root provider version is not pinned to reviewed v5.100.0"

shared_lock="$repo_root/infra/.terraform.lock.hcl"
test -f "$shared_lock" || fail "shared-root provider lock file is missing"
grep -F 'version     = "5.100.0"' "$shared_lock" >/dev/null ||
	fail "shared-root provider lock does not select v5.100.0"
grep -F 'constraints = "5.100.0"' "$shared_lock" >/dev/null ||
	fail "shared-root provider lock does not record the exact constraint"
test "$(grep -Ec '^    "(h1|zh):' "$shared_lock")" -ge 2 ||
	fail "shared-root provider lock lacks reviewed checksums"
test "$(shasum -a 256 "$shared_lock" | awk '{print $1}')" = \
	56bac28a0a2876d61b064ce149df8ba59285a8ede77d1c444cf0b2145ce1ec45 ||
	fail "shared-root provider lock checksum changed"

printf 'PASS: shared root retains only shared data and artifact resources\n'
