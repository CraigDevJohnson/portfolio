...
- [ ] Implement offline-tested plan wrappers before live execution. Reject missing/malformed/symlinked or group/world-readable credential files and existing/relative plan paths. Use private directory/file modes, no shell evaluation, no credential CLI arguments, and no raw plan or provider-error output to stdout. Capture raw plan/JSON/logs only in the private directory. Emit only allowlisted resource-action summaries, account/region/backend identity and checksums.
- [ ] Auth plan checks admit only the five exact auth resources, approved Google/code/public-client/URL/mapping settings, and reviewed create/update/no-op actions. Reject delete/replace, unrelated resources or backend/account/region changes. Add sentinel-secret tests proving no leak to stdout/stderr/public artifacts on success or failure.
- [ ] Implement guarded `cognito-dev-init`, `cognito-dev-plan`, and checksum-protected `cognito-dev-apply` tasks using the repository's exact identity, state-lock and saved-plan conventions. Keep these out of automatic release jobs. Add a public-output export that selects only `management_runtime`, never all outputs or raw state.
- [ ] Refresh `portfolio-deployer` SSO; verify STS account/role, encrypted backend controls, no conflicting Cognito domain, and effective auth-state/Cognito permissions. Current SSO was expired during preflight. Review and separately authorize any required candidate policy installation before using it.
- [ ] Obtain Craig's Google project selection and separately authorize creation of a dedicated web OAuth client. Register `https://portfolio-lambda-dev-mgmt-180294223248.auth.us-west-2.amazoncognito.com/oauth2/idpresponse`, adjusted only for the reviewed domain prefix. Use the confirmed Google address as the test user. Do not reuse Calendar credentials; transfer credentials directly to the private input channel, never chat/Git/tfvars/logs.
...
- [ ] Generate a fresh 32-byte session key and inject its 64-character lowercase hex representation as the exact SecureString through a separately authorized action. Never print or commit it. Verify metadata/access, not the plaintext. Review target EC2 instances before separately authorizing opt-in tagging or boundary installation.
