...
- [ ] Refresh `portfolio-deployer` SSO; verify STS account/role, encrypted backend controls, no conflicting Cognito domain, and effective auth-state/Cognito permissions. The shared SSO session was refreshed September 7; the new non-root `portfolio-auth-policy-admin` profile verified bucket controls, domain availability, candidate validation and matching live policy baselines. Candidate installation and effective setup permissions under `portfolio-deployer` remain pending; see the [external setup review](../../deployment/2026-09-07-cognito-external-setup-review.md).
- [ ] Obtain Craig's Google project selection and separately authorize creation of a dedicated web OAuth client. Project `portoflio-dev-508000` is selected and verified; consent/client creation remains pending. Register `https://portfolio-lambda-dev-mgmt-180294223248.auth.us-west-2.amazoncognito.com/oauth2/idpresponse`, adjusted only for the reviewed domain prefix. Use the confirmed Google address as the test user. Do not reuse Calendar credentials; transfer credentials directly to the private input channel, never chat/Git/tfvars/logs.
