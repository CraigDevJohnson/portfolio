...
- [ ] Obtain Craig's Google project selection and separately authorize creation of a dedicated web OAuth client. Project `portoflio-dev-508000` is selected and verified; the user completed consent configuration and the client form is prepared. Client creation and private credential delivery await Google setup approval. Register `https://portfolio-lambda-dev-mgmt-180294223248.auth.us-west-2.amazoncognito.com/oauth2/idpresponse`, adjusted only for the reviewed domain prefix. Use the confirmed Google address as the test user. Do not reuse Calendar credentials; transfer credentials directly to the private input channel, never chat/Git/tfvars/logs.
