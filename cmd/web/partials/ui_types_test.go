...
func TestClosedVisualVocabularyClasses(t *testing.T) {
	tests := []struct {
		...
		{"action danger", actionVariantClass(ActionDanger), "ui-action-danger"},
		...
		{"stat grid hero", statGridVariantClass(StatGridHero), "stat-grid-hero"},
		{"stat grid compact", statGridVariantClass(StatGridCompact), "stat-grid-compact"},
...
func TestPageCTAPreservesActionVariantsAndPrimaryArrow(t *testing.T) {
	...
	for _, marker := range []string{"ui-action-danger", "ui-action-quiet", `class="btn-icon" aria-hidden="true">→</span>`} {
		if !strings.Contains(html, marker) {
...
func TestActionButtonDisabledLoadingSemantics(t *testing.T) {
	...
	for _, marker := range []string{`disabled`, `aria-disabled="true"`, `aria-busy="true"`} {
		if !strings.Contains(html, marker) {
			t.Errorf("disabled loading ActionButton output does not contain %q: %s", marker, html)
		}
	}
}
...
func TestCardsPreferTypedIconsOverTextGlyphs(t *testing.T) {
	...
	row := renderComponent(t, LinkPanelRow(LinkPanelRowProps{Href: "mailto:test@example.com", Icon: "@", IconName: UIIconMail, Title: "Email"}))
	for name, html := range map[string]string{"feature": feature, "row": row} {
		if !strings.Contains(html, "<svg") || strings.Contains(html, ">legacy<") || strings.Contains(html, ">@<") {
