...
func TestSoccerPageClearsExpiredOrInvalidSessionAndRendersUnauthenticatedState(t *testing.T) {
	app := newTestApp(t)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if lpsStart < 0 {
				t.Fatalf("expected /soccer to render the auth panel, got %q", body)
			}
			lpsEnd := strings.Index(body[lpsStart:], `>`)
			if lpsEnd < 0 || strings.Contains(body[lpsStart:lpsStart+lpsEnd+1], "hx-swap-oob") {
				t.Fatalf("expected /soccer to render the auth panel as primary content, got %q", body)
			}
			if strings.Contains(body, `id="soccer-configuration"`) {
				t.Fatalf("expected /soccer to omit the removed duplicate configuration summary, got %q", body)
			}
			...
			if strings.Contains(resp.Body.String(), `hx-post="/soccer/logout"`) {
