...
func TestTask13Final13DangerRoseBoundaryClearsRenderedFloor(t *testing.T) {
	rose := final10RGB{255, 127, 168}
	cocoa := final10RGB{46, 33, 48}
	dangerSurface := final10Mix(rose, cocoa, 0.16)
	renderedBorder := final10Mix(rose, dangerSurface, 0.72)
	if ratio := final10Contrast(renderedBorder, dangerSurface); ratio < 3 {
		t.Errorf("danger Rose72 modeled adjacent contrast = %.3f, want >= 3", ratio)
	}
}
...
func TestTask13Final13GlyphPseudoPaintMutationGate(t *testing.T) {
	...
	fixtures := []struct {
		...
		{name: "first line color", css: "\n.ui-action-danger::first-line { color: var(--night-mulberry); }\n"},
...
func TestTask13Final13UnsupportedCSSGrammarMutationGate(t *testing.T) {
	...
	fixtures := []struct {
		...
		{name: "native nested danger paint", path: "cmd/web/tailwind/base.css", css: "\n.scope { .ui-action-danger { color: var(--night-mulberry) !important; } }\n"},
...
func TestTask13Final13GeneratedGlyphVariantMutationGate(t *testing.T) {
	for _, fixture := range []string{
		...
		`class="[&::first-line]:text-night-mulberry"`,
		`class="[&_.ui-action-danger::first-line]:text-night-mulberry"`,
		`class="text-shadow-sm"`,
		`class="[text-shadow:0_0.1rem_var(--rosehip)]"`,
		`class="[-webkit-text-fill-color:var(--night-mulberry)]"`,
		`class="[&_[data-task13-contrast-danger-host]]:[-webkit-text-fill-color:var(--night-mulberry)]"`,
