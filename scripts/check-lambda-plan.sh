...
if [ "$AUTOMATED_RELEASE" != false ]; then
  ...
  jq -e \
    ...
    def required_managed_resources: [
      ...
      {address: "module.service.aws_apigatewayv2_route.default", type: "aws_apigatewayv2_route"},
      {address: "module.service.aws_apigatewayv2_stage.default", type: "aws_apigatewayv2_stage"},
      {
        address: "module.service.aws_cloudwatch_log_group.api_access",
        type: "aws_cloudwatch_log_group"
      },
      {address: "module.service.aws_cloudwatch_log_group.lambda", type: "aws_cloudwatch_log_group"},
      {
        address: "module.service.aws_cloudwatch_metric_alarm.api_5xx",
        type: "aws_cloudwatch_metric_alarm"
      },
      {
        address: "module.service.aws_cloudwatch_metric_alarm.api_latency",
        type: "aws_cloudwatch_metric_alarm"
      },
      {
        address: "module.service.aws_cloudwatch_metric_alarm.lambda_duration",
        type: "aws_cloudwatch_metric_alarm"
      },
      {
        address: "module.service.aws_cloudwatch_metric_alarm.lambda_errors",
        type: "aws_cloudwatch_metric_alarm"
      },
      {
        address: "module.service.aws_cloudwatch_metric_alarm.lambda_throttles",
        type: "aws_cloudwatch_metric_alarm"
      },
      {
        address: "module.service.aws_dynamodb_table.google_connections",
        type: "aws_dynamodb_table"
      },
      {address: "module.service.aws_dynamodb_table.soccer_sessions", type: "aws_dynamodb_table"},
      {address: "module.service.aws_iam_role.lambda", type: "aws_iam_role"},
      {address: "module.service.aws_iam_role_policy.lambda", type: "aws_iam_role_policy"},
    ...
    def converged_release($managed):
      all($managed[]; .change.actions == ["no-op"]) and
      [$managed[] | select(.address == "module.service.aws_lambda_function.app")] as $functions |
      [$managed[] | select(.address == "module.service.aws_lambda_alias.live")] as $aliases |
      ($functions | length) == 1 and
      ($aliases | length) == 1 and
      $functions[0] as $function |
      $aliases[0] as $alias |
  ...
  def allowed_managed($address; $type):
    [
      ...
      (if $environment == "dev" then
        ...
        [
          [
            "module.service.aws_apigatewayv2_domain_name.custom[\"craigdevjohnson.com\"]",
            "aws_apigatewayv2_domain_name"
          ],
          [
          ...
          [
            "module.service.aws_apigatewayv2_api_mapping.custom[\"craigdevjohnson.com\"]",
            "aws_apigatewayv2_api_mapping"
          ],
          [
...
jq -e \
  ...
  try (
    ...
    ($kms_resources[0] |
      test("^arn:aws:kms:us-west-2:180294223248:key/([0-9a-f]{8}-){1}([0-9a-f]{4}-){3}[0-9a-f]{12}$")) and
...
jq -e \
  ...
  all(resources("aws_iam_role")[];
    .change.after.name == ($prefix + "-execution") and
    .change.after.permissions_boundary == $boundary) and
  ...
  all(resources("aws_cloudwatch_log_group")[];
    ...
    (.change.after.name == ("/aws/lambda/" + $prefix) or
      .change.after.name == ("/aws/apigateway/" + $prefix + "/access"))) and
  ...
  all(resources("aws_dynamodb_table")[];
    ...
    (.change.after.name == ($prefix + "-google-connections") or
      .change.after.name == ($prefix + "-soccer-sessions"))) and
...
' "$PLAN_JSON" > /dev/null ||
  fail "environment boundary, names, protection, retention, image, or alarm contract drifted"
